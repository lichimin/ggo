...
type Skin struct {
	...
	IdleImageURLs   []string `json:"idle_image_urls" gorm:"type:json"`   // 待机图片（JSON格式多个图片）
	AttackImageURLs []string `json:"attack_image_urls" gorm:"type:json"` // 攻击图片（JSON格式多个图片）
	MoveImageURLs   []string `json:"move_image_urls" gorm:"type:json"`   // 移动图片（JSON格式多个图片）
